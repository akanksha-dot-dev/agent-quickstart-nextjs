...
import { buildLexiPrompt, buildLexiGreeting } from '@/lib/edex-prompt';
import { createDefaultProfile } from '@/lib/learner';

// Fallback prompt when no learner profile is provided (e.g. direct API call)
const FALLBACK_PROMPT = buildLexiPrompt(
  createDefaultProfile('there', 'mathematics', 1, 'sports'),
);
const FALLBACK_GREETING = `Hi! I'm Lexi, your EdexConvoAI tutor. What subject are you studying today?`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { requester_id, channel_name, learnerProfile } = body;
    ...
    // --- 2. Build personalized Lexi prompt ---

    const systemPrompt = learnerProfile
      ? buildLexiPrompt(learnerProfile)
      : FALLBACK_PROMPT;

    const greeting = learnerProfile
      ? buildLexiGreeting(learnerProfile)
      : FALLBACK_GREETING;
